...
Just don't force me to look at it again!
...
void plotDotPart(int x, int y)
{
    if (x >= 0 && x < WIDTH && y >= 0 && y < HEIGHT)
    {
        ...
        if (*d == GRAY7 || *d == SPOT0 || *d == SPOT1 || *d == SPOT2)
            return;
        else if (*d == GRAY6)
            *d = GRAY7;
        else if (*d == GRAY5)
...
void initFormula(int f)
{
    ...
    case (0):
        ...
        break;
    case (1):
        switchpoint += ((34 * 8000) & ~255);
        switchlgt = 20000;
        break;
    case (2):
        switchpoint += ((20 * 8000) & ~255);
        switchlgt = 20000;
        break;
    case (3):
        switchpoint += ((28 * 44100) & ~1023);
        switchlgt = 100000;
        break;
    case (4):
        switchpoint += ((50 * 8000) & ~255);
        switchlgt = 20000;
        break;
    case (5):
        switchpoint += ((34 * 8000) & ~255);
        ...
        break;
    case (6):
        switchpoint += ((30 * 44100) & ~1023);
        switchlgt = 100000;
        break;
    case (7):
        switchpoint += ((38 * 8000) & ~255);
        switchlgt = 20000;
        break;
    case (8):
        switchpoint += ((70 * 8000) & ~255);
        switchlgt = 20000;
        break;
    case (9):
        switchpoint += ((30 * 8000) & ~255);
        switchlgt = 20000;
        break;
    case (10):
        switchpoint += ((50 * 8000) & ~255);
...
int getFromFormula(int f, int t)
{
    ...
    case (8):
        samplerate = 8000;
        t += 16000;
        return ((t * (t >> 12) & (201 * t / 100) & (199 * t / 100)) & (t * (t >> 14) & (t * 301 / 100) & (t * 399 / 100))) + ((t * (t >> 16) & (t * 202 / 100) & (t * 198 / 100)) - (t * (t >> 17) & (t * 302 / 100) & (t * 298 / 100)));
    case (9):
        samplerate = 8000;
        return rygtune[t];
    case (10):
...
void makeTestStuff666()
{
    ...
    static int t = 0;
    {
        int i;
        int pam = 0xF0;
        for (i = 0; i < 40 * 31 * 2; i += 2)
        {
            scr[i + 1] = pam;
            scr[i] = 32;
        }
        if (t > 9 * 30)
            for (i = 0; i < 40 * 31 * 2; i += 40)
            {
                scr[i + 1] = 0x70;
            }
    }
...
void scaleToYUY2()
{
    ...
    for (y = 0; y < HEIGHT - 1; y++)
        for (x = 0; x < WIDTH; x++)
        {
            ...
            oo[2 + WIDTH * 4] = (prevY / 2) | 0;
            prevY = (cY ^ ((r >> 8) & 0x1f)); // (cY ^ (r&0x7));
